...
router.get('/nodes', function(req, res) {
  db.get_peers(function(peers) {
    if(peers) {
      var locations = JSON.stringify(peers);
      res.render('nodes', { active: 'nodes', locations: [locations] });
    } else {
      res.render('nodes', { active: 'nodes', locations: [''] });
    }      
  });
});
...
router.get('/ext/summary', function(req, res) {
  lib.get_difficulty('x16rt', function(difficulty) {
    lib.get_difficulty('minotaurx', function(difficultyMinx) {
      lib.get_hashrate('x16rt', function(hashrate) {
        lib.get_hashrate('minotaurx', function(hashrateMinx) {
            ...
            lib.get_blockcount(function(blockcount) {
              db.get_stats(settings.coin, function (stats) {
                ...
                res.send({ data: [{
                  ...
                  difficultyMinx: difficultyMinx,
                  supply: stats.supply,
                  hashrate: hashrate,
                  hashrateMinx: hashrateMinx,
