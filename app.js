...
app.use('/ext/getblocks/:nbrofblocks', function(req,res){
  db.get_blocks(req.params.nbrofblocks, function(blocks) {
    res.send(blocks);
  })
});
...
app.use('/getserverdate', function(req,res){
  var serverDate = new Date(new Date().toUTCString());
  res.send(serverDate);
});

app.use('/ext/chartdata/:powtype', function(req,res){
  db.get_chart(req.params.powtype, function(data){
    if (data) {
      res.send(data);
    } else {
      res.send()
