...
function find_blocks(nbrofblocks,cb) {
  lib.get_blockcount( function (count) {
    if(!nbrofblocks)
      nbrofblocks = 500;
    if (count){
      Tx.aggregate([{ $match: {blockindex: { $gt:count - nbrofblocks }} },{ $group: { _id: "$powtype", count: { $sum: 1 } } }]).exec(function(err, block) {
        if(block) {
          return cb(block);
        } else {
          return cb();
            ...
            lib.syncLoop(vout.length, function (subloop) {
              ...
              lib.calculate_total(vout, function(total){
                lib.get_block(tx.blockhash, function (block) {
                  if (block) {
                    var powtype = 'x16r';
                    if(block.powtype && block.time > 1638847407) {
                      powtype = block.powtype;
                    }
                    lib.get_hashrate(powtype, function (networkHash) {
                      if (networkHash) {
                        var difficulty =block.difficulty;
                        if(powtype == 'minotaurx')
                          difficulty = block.crowdifficulty;
                        var newTx = new Tx({
                          txid: tx.txid,
                          vin: nvin,
                          vout: vout,
                          ...
                          powtype:  powtype,
                          netHash: networkHash,
                          difficulty: Number.parseFloat(difficulty).toFixed(10),
                        });
...
module.exports = {
  ...
  get_chart: function (powtype, cb) {
    lib.get_blockcount(function (blockcount) {
      var last = parseInt(blockcount);
      console.log("get_chart for %s", powtype);
      Tx.aggregate([{ "$match": { "$and":[ { 'blockindex': { $gt: last - 300, $lt: last } }, { 'powtype': { $eq: powtype } }, ] }}, { $group: { _id: '$blockindex', block: { $first: '$blockindex' }, powdiff: { $first: '$difficulty' }, netHash: { $first: '$netHash' } } }, { $sort: { block: 1 } }]).exec(function (err, pows) {
          if (err) {
          }
          return cb(pows);
      });
  ...
  get_blocks: function(nbrofblocks, cb) {
    find_blocks(nbrofblocks,function(blocks){
      return cb(blocks);
    });
  },
  ...
  get_peers: function(cb) {
      Peers.find({}).sort({ "protocol": -1, "version": -1, "country": 1}).exec(function(err, peers) {
      if (err) {
        return cb([]);
      } else {
        return cb(peers);
