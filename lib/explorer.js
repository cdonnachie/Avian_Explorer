...
module.exports = {
  ...
  get_hashrate: function(powalgo, cb) {
    ...
    if (settings.use_rpc) {
      if (settings.nethash == 'netmhashps') {
        rpcCommand([{method:'getmininginfo', parameters: []}], function(response){
          ...
          if (response.netmhashps) {
            ...
            if (settings.nethash_units == 'K' || powalgo == 'minotaurx') {
        ...
        if(powalgo == 'x16r')
           powalgo = 'x16rt';
        rpcCommand([{method:'getnetworkhashps', parameters: [5, -1, powalgo]}], function(response){
          if (response == 'There was an error. Check your console.') { return cb(response);}
            if (response) {
      ...
      if (settings.nethash == 'netmhashps') {
        ...
        if(powalgo != 'x16r')
          uri = uri + '?nblocks=5&height=-1&powalgo=' + powalgo;
  ...
  get_difficulty: function(powalgo, cb) {
    if (settings.use_rpc) {
      if(powalgo == 'x16r')
        powalgo = '';
      rpcCommand([{method:'getdifficulty', parameters: [powalgo]}], function(response){
        return cb(response);
      });
    } else {
      var uri = base_url + 'getdifficulty';
      if(powalgo != 'x16r')
        uri = uri + '?powalgo=' + powalgo;
      request({uri: uri, json: true}, function (error, response, body) {
        return cb(body);
      });
    }
